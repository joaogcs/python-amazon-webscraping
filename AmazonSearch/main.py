...
class App(AttributeManager):
    ...
    def run(self):
        ...
        self.attributes['webdriver'] = webdriver.Chrome(options=options)
...
class AmazomSearch(App):
    def __init__(self, silent_mode=False, timeout=5, **kwargs):
        ...
        super(AmazomSearch, self).__init__(silent_mode=silent_mode, timeout=timeout)
    ...
    def save_search(self, list_to_save, file_name='search', delimiter=','):
        ...
        file_name = file_name + dt_string + '.csv'
...
def main():
    ...
    amazonsearch = AmazomSearch(silent_mode=False)
    try:
        # Create AmazonSearch app. Optional silent_mode (Default is False)
        amazonsearch.run()
        ...
        list_of_items = amazonsearch.get_items_through_pages(number_of_pages=1)
